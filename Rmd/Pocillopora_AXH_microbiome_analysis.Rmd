...
ggdepth +
  ...
  scale_fill_manual(values = c(condfillcolors_.AXH, "dark gray", "light gray")) +
  ylab("16S Read Count") +
  theme(axis.text.x = element_text(angle = 90)) +
  ggtitle("Total number of 16S reads per sample")
ggsave(file = "./outputs/phyloseq-results/figures/readdepth_.AXH.png", device = "png", height = 5, width = 8.5, units = "in")
```
...
grare + 
  ...
  scale_fill_manual(values = condfillcolors_.AXH) +
  #facet_wrap(~Treatment) +
  ggtitle("Rarefaction curve of 16S amplicon sequence variants")
ggsave(file = "./outputs/phyloseq-results/figures/rarefaction_.AXH.png", device = "png", height = 5, width = 8.5, units = "in")
```
...
```{r}
pdf(file = "./outputs/phyloseq-results/figures/barplot_order_.AXH.pdf", height = 8.5, width = 11)
...
```
```{r}
#pdf(file = "./outputs/phyloseq-results/figures/barplot_family_.AXH.pdf", height = 8.5, width = 11)

length(unique(ps.AXH_family$Family))
...
### samples collapsed into colonies
png( filename = "./manuscript_figures/Fig3B_RelAbundBarplot.png", height = 5, width = 8, units = 'in', res = 150)
...
print(gf2)
dev.off()
#ggsave(print(gf2), filename = "./manuscript_figures/Fig3B_RelAbundBarplot.png")#, height = "5", width = "8", units = "in", device = "png")
...
```
```{r}
#pdf(file = "./outputs/phyloseq-results/figures/richness_chao1_.AXH.pdf", height = 3, width = 2.8)
chao1 <- plot_richness(ps.AXH, x="Treatment", measures=c("Chao1")) +
  ...
  ggtitle("Bacteria Community Alpha-Diversity") +
  theme(legend.position = "none",
          axis.text.x = element_text(angle = 0),
        strip.background = element_blank(),
        strip.text = element_blank())
...
```
```{r}
pdf(file = "./outputs/phyloseq-results/figures/richness_shannon_.AXH.pdf", height = 3, width = 2.8)
plot_richness(ps.AXH, x="Treatment", measures=c("Shannon")) +
  geom_boxplot(aes(fill = Treatment)) +
  geom_point(aes(color = Treatment, shape = Colony), size = 3, alpha = 0.8) +
  scale_color_manual(values = condcolors_AXH) +
  scale_fill_manual(values = condfillcolors_AXH) +
...
```
```{r}
pdf(file = "./outputs/phyloseq-results/figures/richness_simpson_.AXH.pdf", height = 3, width = 2.8)
plot_richness(ps.AXH, x="Treatment", measures=c("Simpson")) +
  geom_boxplot(aes(fill = Treatment)) +
  geom_point(aes(color = Treatment, shape = Colony), size = 3, alpha = 0.8) +
  scale_color_manual(values = condcolors_AXH) +
  scale_fill_manual(values = condfillcolors_AXH) +
...
```
```{r}
ps_richness_meta <- cbind(ps.AXH_richness, map1)
```
...
p
ggsave(filename = "./outputs/phyloseq-results/figures/ordinations_bray.png", height = 8, width = 10, units = "in")
```
```{r}
plist[[6]] + 
...
#### Specific ordination plots for AXH paper 
...
ord_trmt <- ord %>%
  ...
  full_join(ord)
# Plot with ellipses qnd spiders
pcoa3 <- plot_ordination(ps.AXH, ps.AXH.ord.PCoA.brayP, type = "sample", axes = c(1,2)) +
  #treatment ellipses
  #stat_ellipse(aes(color = Treatment, fill = Treatment), geom = "polygon", type = "norm", alpha = 0.0) + 
  #sample-centroid spiders paths
   geom_segment(data = ord_trmt, mapping = aes(x = `Axis.1`, y = `Axis.2`, xend = c1, yend = c2),
               lwd = 0.25, col = "dark grey") +
  #treatment centroid points
  geom_point(data = ord_trmt, size = 4, aes(x = c1, y = c2, color = Treatment), fill = "black", shape = 21, stroke = 2, show.legend = TRUE) +
  #sample points
  geom_point(size = 3, aes(fill = Treatment, shape = Colony), color = "black", stroke = 0.5, show.legend = FALSE) +
...
pcoa3
ggsave(pcoa3, filename = "./outputs/phyloseq-results/figures/pcoa_treatment.png", width = 6, height = 4, units = "in")
ggsave(pcoa3, filename = "./manuscript_figures/Fig3A_PCoA_16S_Treatment.png", width = 6, height = 4, units = "in")
```
```{r}
combo_pcoa <- pcoa2 | pcoa3
ggsave(combo_pcoa, filename = "./manuscript_figures/Fig_PCoA_RNAseq-16S_Treatment.png", width = 8, height = 4, units = "in")
```
```{r}
figure3 <- pcoa3 | (chao1 / p_family2)
ggsave(figure3, filename = "./manuscript_figures/Fig3_16S_panel.png", width = 14, height = 8, units = "in")
```
...
NMDS
```{r}
p1 <- plot_ordination(ps.AXH.rel, ps.AXH.ord.NMDS.brayP, type = "sample", axes = c(1,2)) +
...
```
```{r}
p1 <- plot_ordination(ps.AXH, ps.AXH.ord.NMDS.brayP, type = "sample", axes = c(1,2)) +
  stat_ellipse(aes(fill = Colony), geom = "polygon", type = "norm", alpha = 0.6) + 
  geom_point(size = 6, aes(color = Treatment, shape = Colony)) +
  scale_color_manual(values = condcolors_AXH, name="Treatment") +
  scale_fill_manual(values = colcolors, name = "Colony") +
...
```{r}
dis <- vegdist(otu_table(ps.AXH.rel), method ="bray")
