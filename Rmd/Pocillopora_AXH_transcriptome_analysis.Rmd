...
# Retaining 11 PC's gives highest a-score (25%), but only utilizes 44% of variance. Retaining 15 PC's reduces a-score to 17%, but retains 72% of variance. Therefore, let's use 15 PC's for the DA. 
...
Should I filter genes for expression even more strictly before WGCNA? 
Which criteria should I use? Differential expression (developers say don't do this!), higher expression?
...
mergeCutHeight = 0.3 because I want to group together modules 
...
Following the tutorial as in: https://horvath.genetics.ucla.edu/html/CoexpressionNetwork/Rpackages/WGCNA/Tutorials/FemaleLiver-02-networkConstr-man.pdf
```{r adjacency_TOM}
...
```
```{r gene_dendrogram}
# may need to delete adjacency, TOM to clear up vector memory
...
## WGCNA module connectivity scatterplots, hubgenes, and edge files
```{r}
geneModuleMembership <- as.data.frame(cor(datExpr, MEs1, use = "p"))
### gene trait significance for each treatment, genotype, etc.
geneTraitSignificance <- as.data.frame(cor(datExpr, datTraits$control, use = "p"))
```
```{r}
GS <- as.numeric(cor(datExpr, datTraits$Antibiotics.Heat, use = "p"))
  GScolor <- numbers2colors(GS, signed = T)
  numberOfColumn <- substring(colnames(datKME[2:22]), 4)
```
```{r}
 for (module in uniqModules) {
  column <- match(module, numberOfColumn)
  restModule <- moduleColors == module
  verboseScatterplot(datKME[restModule, 4], GS[restModule], 
                     ylim = c(-1,1),
                     xlim = c(-0.5,1),
                     abline = TRUE,
                     col = module,
                     ...
                     ylab = paste("Control Treatment Gene Significance"),
                     main = module) 
  }
```
```{r kme_scatterplots}
...
g <- hikME %>% ggplot(., aes(GS, kME)) + geom_point(color = module) + 
