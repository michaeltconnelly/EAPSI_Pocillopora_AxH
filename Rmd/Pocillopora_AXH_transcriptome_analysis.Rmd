...
```
```{r gene data}
gene_annotation <- read.delim(file = "./data/pdam_genome_annotations.tsv", header = T) %>% arrange(ID)
...
pcoa1 <- ggplot(mds_col, aes(fill = Treatment)) +
  ...
  geom_point(size = 3, aes(x = c1, y = c2, shape = Colony), stroke = 2, fill = "black", show.legend = TRUE) + #color = Colony,
  # sample points
...
pcoa1
# ggsave(pcoa1, filename = "./outputs/DESeq-results/figures/pcoa_colony.pdf", width = 112, height = 80, units = "mm", device = "pdf")
...
DEtab <- DE %>%
  ...
  dplyr::select(Colony, num, den, `DEGs [up, down]`)
...
```{r experimental}
DEres %>% 
  filter(ID %in% dpvarc$ID[dpvarc$LD1 > 0.0005]) %>% 
  filter(padj_Antibiotics_control < 0.05, padj_Heat_control < 0.1) %>% 
  filter(log2FoldChange_Heat_control > 0, log2FoldChange_Antibiotics.Heat_control > 0) %>% View()
  # select(-contains("Heat_control")) %>% 
  ggplot(aes(x = log2FoldChange_Heat_control, y = log2FoldChange_Antibiotics.Heat_control)) + geom_point()
```
...
```
```{r treatment_venn_up}
pdf(file = "./outputs/DESeq-results/figures/Venn_AXH_up.pdf", height = 4, width = 4)
vp = viewport(height = unit(4, "in"),  width = unit(4, "in"))
grid.newpage()
grid.rect(vp = vp, gp = gpar(col = "white"))
...
treatment_euler_up <- plot(fit_up,
     ...
     adjust_labels = TRUE)
treatment_euler_up
ggsave(plot = treatment_euler_up, filename = "./outputs/DESeq-results/figures/Euler_AXH_up.pdf", width = 80, height = 80, units = "mm", device = "pdf")
```
...
pdf(file = "./outputs/DESeq-results/figures/Venn_AXH_down.pdf", height = 4, width = 4)
vp = viewport(height = unit(4, "in"),  width = unit(4, "in"))
grid.newpage()
grid.rect(vp = vp, gp = gpar(col = "white"))
...
ggsave(plot = treatment_euler_down, filename = "./outputs/DESeq-results/figures/Euler_AXH_down.pdf", width = 80, height = 80, units = "mm", device = "pdf")
```
...
ggsave(filename = "./outputs/DESeq-results/KOGMWU/figures/KOG_treatment_heatmap.pdf", KOG_treatments, width = 112, height = 150, units = "mm")
...
datTaxa_Family <- datTaxa_Family %>% dplyr::select(-Alteromonadaceae, -Endozoicomonadaceae)
```
...
intModules <- c("steelblue", "ivory", "darkolivegreen", "green", "white", "darkgreen", "purple")
for (module in intModules[7]){
  print(module)
...
##01
commandArgs <- function(...) c(tab, "BP", fig, module, 0.25, 0.1, 0.05, 1.0)
source("./R/GO_MWU/GO_MWU_WGCNA_plot.R")
#}
```
```{r modules_GO}
modules.GO <- as.list(
  c(greenyellow = "./R/GO_MWU/MWU_BP_greenyellow_kme.txt",
    ...
    steelblue = "./R/GO_MWU/MWU_BP_steelblue_kme.txt",
    ivory = "./R/GO_MWU/MWU_BP_ivory_kme.txt",
    green = "./R/GO_MWU/MWU_BP_green_kme.txt",
    darkgreen = "./R/GO_MWU/MWU_BP_darkgreen_kme.txt",
    white = "./R/GO_MWU/MWU_BP_white_kme.txt",
    purple = "./R/GO_MWU/MWU_BP_purple_kme.txt")
  ) %>% map(read.table, header = T) %>%
  bind_rows(.id = "module") %>%
  as_tibble()
...
modules.GO %>%
  ...
  slice_head(n = 15) %>% 
  write_excel_csv(path = "./outputs/WGCNA-results/GO_MWU/ivory.GO.csv")
...
pdf("./outputs/WGCNA-results/combo_heatmap.pdf", height = 8.5, width = 11)
...
ggsave(sigmods_kME_GS_scatterplot, filename = "./outputs/WGCNA-results/figures/sigmodules_kME_gene_significance_scatterplot.pdf", width = 8.5, height = 11, device = "pdf")
```
```{r focal_kME_GS_scatterplots}
# want to hide the ugly parts of this in the functions files
module <- "darkorange"
pdf(file = "./outputs/WGCNA-results/figures/sigmodules_kME_gene_significance_scatterplots", width = 6, height = 4)
for(module in sigModules) {
