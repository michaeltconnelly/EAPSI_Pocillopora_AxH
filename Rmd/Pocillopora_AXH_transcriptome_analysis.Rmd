...
pcoa2 <- ggplot(mds_trmt, aes(fill = Treatment)) +
  ...
  guides(color = guide_legend(override.aes = list(color = condcolors_AxH, alpha = 1, stroke = 1)),
         fill = guide_legend(override.aes = list(fill = condcolors_AxH, alpha = 1, stroke = 1)),
...
DEres <- DE %>%
  ...
  select(contrast, ID, log2FoldChange, padj) %>% 
...
###
commandArgs <- function(...) c(tab, "BP", fig, module, 0.1, 0.05, 0.01, 1.0)
source("./R/GO_MWU/GO_MWU_WGCNA_plot.R")
#}
```
```{r modules_GO}
...
sigmods_kME_GS_scatterplot <- tidy_gene_GS_kME %>% 
  ...
  scale_color_manual(values = sigModules) +
