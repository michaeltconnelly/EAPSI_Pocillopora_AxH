...
```{r}
pdf(file = "~/computing/scripts/EAPSI_AntiXHeat-master/DESeqresults_Pdam/figures/Heatmap_AXH_samples_1.pdf", width = 8.5, height = 7.5)
...
```{r}
TRAFs <- scan("./data/pdam_TRAFs.txt", what = "character")
```
```{r}
mat_TRAF <- assay(rld_AxH)[row.names(rld_AxH) %in% TRAFs, ]
range(mat_TRAF)
mat_TRAF <- mat_TRAF - rowMeans(mat_TRAF)
range(mat_TRAF)
```
```{r}
pdf(file = "./figures/Heatmap_TRAFs.pdf", width = 8, height = 6)
pheatmap(mat_TRAF, col=geneexpcolors,
