...
```{r}
colnames(countdata_vst) <- stable_samples$sample
```
...
ncol(datExpr)
```
This returns 17387 genes for network analysis
...
write.csv(hubs, file = "./WGCNAmodules_Pdam/hubgenes/hubs.csv")
```
## Search module gene notes from ID lists
```{bash}
...
```{r}
datTraits <- select(stable_samples, -treatment, -genotype) 
...
```{r}
setwd("./WGCNAmodules_Pdam_ctrl-LPS")
