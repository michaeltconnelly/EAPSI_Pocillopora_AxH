...
keep_tencounts <-  rowSums(counts(dds_coral)) >= 10
keep_ninety <- rowSums(counts(dds_coral) >= 10) > (47*0.9)
dds_coral <- dds_coral[keep_ninety, ]
...
# Subset DESeqDataSet
# Remove genes with counts less than 10 in 90% of samples
keep_tencounts <-  rowSums(counts(dds_symbiont)) >= 10
keep_ninety <- rowSums(counts(dds_symbiont) >= 10) > (47*0.9)
dds_symbiont <- dds_symbiont[keep_ninety, ]
# Normalize expression data for visualization purposes using VST tranformation
...
DE_table <- cbind(DEtab_coral, DEtab_symbiont$`DEGs [up, down]`)
# Filter only contrasts against the control treatment
DE_table %>%
  filter(den == "control") %>% 
