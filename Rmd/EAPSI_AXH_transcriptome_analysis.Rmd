...
shared_DEGs_treatments <- DE %>%
  ...
  filter(den == "control") %>% 
  group_by(Colony) %>%
  dplyr::count(ID, log2FoldChange > 0) %>% 
...
DE_results_coral %>% 
  filter(padj_Heat_control < 0.05 | padj_Antibiotics_control < 0.05 | padj_Antibiotics.Heat_control < 0.05) %>% 
  filter(DE_Heat == DE_Antibiotics.Heat & DE_Heat != "NA") %>% dplyr::count()
```
```{r}
DE_results_sig_coral %>% filter(is.na(DE_Antibiotics)) %>% View()
  dplyr::count(DE_Heat_Antibiotics)
```

```{r}
DE_case_results <- DE_results_sig_coral %>%
  mutate(DE_Heat_Antibiotics = ifelse(DE_Heat == "UP" & DE_Antibiotics == "UP", "UP_UP", 
                                           ifelse(DE_Heat == "UP" & DE_Antibiotics == "DOWN", "UP_DOWN",
                                                  ifelse(DE_Heat == "DOWN" & DE_Antibiotics == "UP", "DOWN_UP", 
                                                         ifelse(DE_Heat == "DOWN" & DE_Antibiotics == "DOWN", "DOWN_DOWN", "NA")))),
         #
         DE_Heat_Antibiotics.Heat = ifelse(DE_Heat == "UP" & DE_Antibiotics.Heat == "UP", "UP_UP", 
                                           ifelse(DE_Heat == "UP" & DE_Antibiotics.Heat == "DOWN", "UP_DOWN",
                                                  ifelse(DE_Heat == "DOWN" & DE_Antibiotics.Heat == "UP", "DOWN_UP", 
                                                         ifelse(DE_Heat == "DOWN" & DE_Antibiotics.Heat == "DOWN", "DOWN_DOWN", "NA")))),
         #
         DE_Antibiotics_Antibiotics.Heat = ifelse(DE_Antibiotics == "UP" & DE_Antibiotics.Heat == "UP", "UP_UP", 
                                           ifelse(DE_Antibiotics == "UP" & DE_Antibiotics.Heat == "DOWN", "UP_DOWN",
                                                  ifelse(DE_Antibiotics == "DOWN" & DE_Antibiotics.Heat == "UP", "DOWN_UP", 
                                                         ifelse(DE_Antibiotics == "DOWN" & DE_Antibiotics.Heat == "DOWN", "DOWN_DOWN", "NA"))))) %>% 
  
  mutate(DE_All = ifelse(DE_Heat != "NA" & DE_Antibiotics != "NA" & DE_Antibiotics.Heat != "NA", "TRUE", "FALSE")) %>% 
  
  select(ID, contains("DE"))
```
```{r}
DE_case_results %>% filter(is.na(DE_Antibiotics)) %>% View()
```
...
logPfilepath <- paste0("./R/GO_MWU/logP", "_", "heat.control", "_", partner, ".txt")
DE %>%
...
#
logPfile <- paste0("logP", "_", "heat.control", "_", partner, ".txt")
```
```{r heat GO_MWU}
# Run GO_MWU for Heat vs. control for Biological Process (BP) GO terms
commandArgs <- function(...) c(logPfile, "BP")
source("./R/GO_MWU/GO_MWU.R")

# Run GO_MWU for Heat vs. control for Molecular Function GO terms
commandArgs <- function(...) c(logPfile, "MF")
source("./R/GO_MWU/GO_MWU.R")

# Run GO_MWU for Heat vs. control for Cellular Component (CC) GO terms
commandArgs <- function(...) c(logPfile, "CC")
source("./R/GO_MWU/GO_MWU.R")
```
```{r heat.control_GO}
heat.control.GO <- as.list(
  c(BP = "./R/GO_MWU/MWU_BP_logP_heat.control_coral.txt",
    MF = "./R/GO_MWU/MWU_MF_logP_heat.control_coral.txt",
    CC = "./R/GO_MWU/MWU_CC_logP_heat.control_coral.txt")
  ) %>% map(read.table, header = T) %>%
  bind_rows(.id = "ontology") %>%
  as_tibble() %>%
  filter(p.adj < 0.05)
#
heat.control.GO_symbiont <- as.list(
  c(BP = "./R/GO_MWU/MWU_BP_logP_heat.control_symbiont.txt",
    MF = "./R/GO_MWU/MWU_MF_logP_heat.control_symbiont.txt",
    CC = "./R/GO_MWU/MWU_CC_logP_heat.control_symbiont.txt")
  ) %>% map(read.table, header = T) %>%
  bind_rows(.id = "ontology") %>%
  as_tibble() %>%
  filter(p.adj < 0.05)
...
heat.control.GO %>% filter(p.adj < 0.05 & delta.rank >= 0) %>% 
  group_by(ontology) %>% 
  summarise(n = n())
```
```{r heat GO_MWU plot}
...
anti.control.GO <- as.list(
  ...
  filter(p.adj < 0.05)
#
anti.control.GO_symbiont <- as.list(
  c(BP = "./R/GO_MWU/MWU_BP_logP_antibiotics.control_symbiont.txt",
    MF = "./R/GO_MWU/MWU_MF_logP_antibiotics.control_symbiont.txt",
    CC = "./R/GO_MWU/MWU_CC_logP_antibiotics.control_symbiont.txt")
  ) %>% map(read.table, header = T) %>%
  bind_rows(.id = "ontology") %>%
  as_tibble() %>%
  filter(p.adj < 0.05)
...
anti.control.GO %>% filter(p.adj < 0.05 & delta.rank <= 0) %>% 
  group_by(ontology) %>% 
  summarise(n = n())
```
```{r anti GO_MWU plot}
...
logPfilepath <- paste0("./R/GO_MWU/logP", "_", "antibiotics.heat.control", "_", partner, ".txt")
DE %>%
  ...
  write.table(., file = logPfilepath, row.names = FALSE, quote = FALSE, sep = ",")
#
logPfile <- paste0("logP", "_", "antibiotics.heat.control", "_", partner, ".txt")
```
```{r anti.heat GO_MWU}
...
anti.heat.control.GO <- as.list(
  c(BP = "./R/GO_MWU/MWU_BP_logP_antibiotics.heat.control_coral.txt",
    MF = "./R/GO_MWU/MWU_MF_logP_antibiotics.heat.control_coral.txt",
    CC = "./R/GO_MWU/MWU_CC_logP_antibiotics.heat.control_coral.txt")
  ) %>% map(read.table, header = T) %>%
  bind_rows(.id = "ontology") %>%
  as_tibble() %>%
  filter(p.adj < 0.05)
#
anti.heat.control.GO_symbiont <- as.list(
  c(BP = "./R/GO_MWU/MWU_BP_logP_antibiotics.heat.control_symbiont.txt",
    MF = "./R/GO_MWU/MWU_MF_logP_antibiotics.heat.control_symbiont.txt",
    CC = "./R/GO_MWU/MWU_CC_logP_antibiotics.heat.control_symbiont.txt")
  ) %>% map(read.table, header = T) %>%
  bind_rows(.id = "ontology") %>%
  as_tibble() %>%
  filter(p.adj < 0.05)
...
anti.heat.control.GO %>% filter(p.adj < 0.05 & delta.rank >= 0 & ontology == "BP") %>% 
  group_by(ontology) %>% 
  summarise(n = n())
```
...
write_csv(allGO, path = "./outputs/DESeq_results/GO_MWU/allGO_coral.csv")

allGO_symbiont <- bind_rows(.id = "contrast",
  heat.control = heat.control.GO_symbiont,
  anti.control = anti.control.GO_symbiont,
  anti.heat.control = anti.heat.control.GO_symbiont
) %>%
  select(contrast, ontology, delta.rank, name, p.adj) %>%
  arrange(contrast, ontology, sign(delta.rank), p.adj)

write_csv(allGO_symbiont, path = "./outputs/DESeq_results/GO_MWU/allGO_symbiont.csv")
```
...
ggsave(filename = "./outputs/DESeq_results/KOGMWU/figures/KOG_treatment_heatmap_symbiont.pdf", KOG_treatments, width = 112, height = 92, units = "mm")
...
pdf(file = "./outputs/WGCNA_results/dendrogram_symbiont.pdf", width = 8, height = 6)
...
write_csv(hub_genes, path = "./outputs/WGCNA_results/hub_genes_symbiont.csv")
```
```{r sig_hubs}
sig_hubs <- hub_genes %>%
...
# write_csv(sig_hubs, path = "./outputs/WGCNA_results/sig_hub_genes_table.csv")
write_excel_csv(sig_hubs, path = "./manuscript_figures/Table2_symbiont.csv")
```
```{r datkME}
...
# pdf(file = "./outputs/WGCNA-results/figures/ME_complexheatmap_treatments.pdf", width = 6.5, height = 6)
pdf(file = "./manuscript_figures/Fig4B_ME_heatmap_symbiont.pdf", width = 6, height = 6)
...
# me_treatment_heatmap_coral <- me_treatment_heatmap
# me_treatment_heatmap_symbiont <- me_treatment_heatmap
# draw(me_treatment_heatmap_coral,
#      show_annotation_legend = FALSE, heatmap_legend_side = "left"
#      )
