...
samples.AxC <- grep("[HW][wt][1-2].[46][abc]", samples, value = TRUE)
...
```
```{r}
write.csv(x.ps, file = "./outputs/anti-aldex2.csv")
```
```{r}
...
aldex1 <- aldex1 + ggtitle(subtitle = "Heat vs. Control Treatment", "Bacteria ASV ALDeX2 Differential Abundance MW/MA Plots") 
...
aldexplots
ggsave(aldexplots, filename = "./outputs/aldex2/heat-asv-aldex2.pdf", device = "pdf")
```
```{r}
cutoff <- 0.1
